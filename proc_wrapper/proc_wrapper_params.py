...
from .common_utils import coalesce, string_to_bool, string_to_int, encode_int
...
class ConfigResolverParams:
    ...
    def override_resolver_params_from_env(self, env: Dict[str, str]) -> None:
        ...
        if env_locations_in_env is not None:
            self.env_locations = self.split_location_string(
                    env_locations_in_env)
...
class ProcWrapperParams(ConfigResolverParams):
    def __init__(self, embedded_mode: bool=True):
        ...
        self.api_heartbeat_interval: Optional[int] = \
        ...
        self.process_check_interval: int = \
                DEFAULT_PROCESS_CHECK_INTERVAL_SECONDS
        self.process_termination_grace_period: Optional[int] = \
    ...
    def override_proc_wrapper_params_from_env(self, env: Dict[str, str],
    ...
    def populate_env(self, env: Dict[str, str]) -> None:
        ...
        if not self.offline_mode:
            ...
            env['PROC_WRAPPER_API_ERROR_TIMEOUT_SECONDS'] = \
                    str(encode_int(self.api_error_timeout, empty_value=-1))
            env['PROC_WRAPPER_API_RETRY_DELAY_SECONDS'] = \
                    str(self.api_retry_delay)
            env['PROC_WRAPPER_API_RESUME_DELAY_SECONDS'] = \
                    str(encode_int(self.api_resume_delay))
            env['PROC_WRAPPER_API_REQUEST_TIMEOUT_SECONDS'] = \
                    str(encode_int(self.api_request_timeout, empty_value=-1))

            enable_status_update_listener = self.enable_status_update_listener
            env['PROC_WRAPPER_ENABLE_STATUS_UPDATE_LISTENER'] = \
                    str(enable_status_update_listener).upper()
            if enable_status_update_listener:
                env['PROC_WRAPPER_STATUS_UPDATE_SOCKET_PORT'] = \
        ...
        if self.rollbar_access_token:
            ...
            env['PROC_WRAPPER_ROLLBAR_RETRY_DELAY_SECONDS'] = str(self.rollbar_retry_delay)
        ...
        if self.task_instance_metadata:
            env['PROC_WRAPPER_TASK_INSTANCE_METADATA'] = \
                json.dumps(self.task_instance_metadata)

        env['PROC_WRAPPER_PROCESS_TIMEOUT_SECONDS'] = str(encode_int(
        ...
        env['PROC_WRAPPER_MAX_CONCURRENCY'] = str(encode_int(
    ...
    def _override_immutable_from_env(self, env: Dict[str, str],
        ...
        self.task_version_number = coalesce(string_to_int(
                env.get('PROC_WRAPPER_TASK_VERSION_NUMBER')),
                self.task_version_number)

        self.task_version_text = env.get('PROC_WRAPPER_TASK_VERSION_TEXT',
        ...
        self.auto_create_task = (self.auto_create_task_props is not None) \
                or coalesce(string_to_bool(
                env.get('PROC_WRAPPER_AUTO_CREATE_TASK')),
                auto_create_task_props.get('was_auto_created'),
                self.auto_create_task)

        max_concurrency = string_to_int(
                env.get('PROC_WRAPPER_TASK_MAX_CONCURRENCY'),
                negative_value=-1)
        ...
        if max_concurrency is None:
            ...
            self.max_concurrency = coalesce(max_concurrency,
                    self.max_concurrency)

        args_forced_passive = None if (self.force_task_active is None) \
                else (not self.force_task_active)
        ...
        if self.auto_create_task:
            ...
            self.task_is_passive = coalesce(string_to_bool(
                    env.get('PROC_WRAPPER_TASK_IS_PASSIVE')),
                    args_forced_passive) or False

        self.task_execution_uuid = env.get('PROC_WRAPPER_TASK_EXECUTION_UUID',
                self.task_execution_uuid)

        self.task_uuid = env.get('PROC_WRAPPER_TASK_UUID',
        ...
        self.enable_status_update_listener = string_to_bool(
                env.get('PROC_WRAPPER_ENABLE_STATUS_UPDATE_LISTENER'),
                default_value=self.enable_status_update_listener) or False

        if self.enable_status_update_listener:
            self.status_update_socket_port = string_to_int(
            ...
            self.status_update_message_max_bytes = string_to_int(
    ...
    def _override_mutable_from_env(self, env: Dict[str, str]) -> None:
        ...
        if self.rollbar_access_token:
            ...
            self.rollbar_retry_delay = coalesce(string_to_int(
                    env.get('PROC_WRAPPER_ROLLBAR_RETRY_DELAY_SECONDS')),
                    self.rollbar_retry_delay)

            self.rollbar_timeout = coalesce(string_to_int(
        ...
        self.process_max_retries = cast(int, coalesce(string_to_int(
                ...
                negative_value=0), self.process_max_retries))

        self.process_retry_delay = cast(int, coalesce(string_to_int(
                env.get('PROC_WRAPPER_PROCESS_RETRY_DELAY_SECONDS'),
                negative_value=0), self.process_retry_delay))
        ...
        task_instance_metadata_str = env.get(
        ...
        if task_instance_metadata_str:
            try:
                ...
                _logger.exception(f"Failed to parse instance metadata: '{task_instance_metadata_str}'")
        ...
        default_heartbeat_interval: Optional[int] = \
                DEFAULT_API_HEARTBEAT_INTERVAL_SECONDS

        is_concurrency_limited_service = self.service \
        ...
        self.api_task_execution_creation_conflict_timeout = string_to_int(
                env.get('PROC_WRAPPER_API_TASK_EXECUTION_CREATION_CONFLICT_TIMEOUT_SECONDS'),
        ...
        self.api_task_execution_creation_conflict_retry_delay = string_to_int(
                env.get('PROC_WRAPPER_API_TASK_EXECUTION_CREATION_CONFLICT_RETRY_DELAY_SECONDS'),
                default_value=default_task_execution_creation_conflict_retry_delay) or 0
        ...
        self.send_pid = string_to_bool(env.get('PROC_WRAPPER_SEND_PID'),
                default_value=self.send_pid) or False
...
environment.
    ...
    task_group.add_argument('-n', '--task-name', help="""
Name of Task (either the Task Name or the Task UUID must be specified""")
    task_group.add_argument('--task-uuid', help="""
UUID of Task (either the Task Name or the Task UUID must be specified)""")
    task_group.add_argument('-a', '--auto-create-task', action='store_true',
            help='Create the Task even if not known by the API server')
    task_group.add_argument('--auto-create-task-run-environment-name',
    ...
    task_group.add_argument('--auto-create-task-run-environment-uuid', help="""
...
UUID of the Run Environment must be specified if auto-creating the Task)""")
    task_group.add_argument('--auto-create-task-props', type=json_encoded,
            help="""
...
schema.""")
    task_group.add_argument('--force-task-active', action='store_const',
            const=True, help="""
...
passive.""")
    task_group.add_argument('--task-execution-uuid',
            help='UUID of Task Execution to attach to')
    task_group.add_argument('--task-version-number',
            help="Numeric version of the Task's source code")
    task_group.add_argument('--task-version-text',
            help="Human readable version of the Task's source code")
    task_group.add_argument('--task-version-signature',
            help="""
Version signature of the Task's source code (such as a git commit hash)""")
    task_group.add_argument('--execution-method-props', type=json_encoded,
            help="""
...
for the schema.""")
    task_group.add_argument('--task-instance-metadata', type=json_encoded,
            help="Additional metadata about the Task instance, in JSON format")
    task_group.add_argument('-s', '--service', action='store_true',
            help='Indicate that this is a Task that should run indefinitely')
    task_group.add_argument('--schedule',
            help='Run schedule reported to the API server')
    task_group.add_argument('--max-concurrency',
            help="""
Maximum number of concurrent Task Executions allowed with the same Task UUID.
Defaults to 1.""")
    task_group.add_argument('--max-conflicting-age',
...
seconds for services that send heartbeats. Otherwise, defaults to no limit.""")
    ...
    api_group.add_argument('--api-base-url', default=DEFAULT_API_BASE_URL,
            help=f'Base URL of API server. Defaults to {DEFAULT_API_BASE_URL}')
    api_group.add_argument('-k', '--api-key',
            help="""
API key. Must have at least the Task access level, or Developer access level for
auto-created Tasks.""")
    api_group.add_argument('--api-heartbeat-interval',
            default=UNSET_INT_VALUE,
            help=f"""
...
otherwise.""")
    api_group.add_argument('--api-error-timeout',
...
server. Defaults to {DEFAULT_API_ERROR_TIMEOUT_SECONDS}.""")
    api_group.add_argument('--api-final-update-timeout',
...
{DEFAULT_API_FINAL_UPDATE_TIMEOUT_SECONDS}.""")
    api_group.add_argument('--api-retry-delay',
            default=DEFAULT_API_FINAL_UPDATE_TIMEOUT_SECONDS,
            help=f"""
Number of seconds to wait before retrying an API request.
Defaults to {DEFAULT_API_RETRY_DELAY_SECONDS}.""")
    api_group.add_argument('--api-resume-delay',
            default=DEFAULT_API_RESUME_DELAY_SECONDS, help=f"""
...
-1 means no resumption.""")
    api_group.add_argument('--api-task-execution-creation-error-timeout',
            help=f"""
...
Defaults to {DEFAULT_API_TASK_EXECUTION_CREATION_TIMEOUT_SECONDS}.""")
    api_group.add_argument('--api-task-execution-creation-conflict-timeout',
...
concurrency limited services, 0 otherwise.""")
    api_group.add_argument('--api-task-execution-creation-conflict-retry-delay',
            default=UNSET_INT_VALUE, help=f"""
...
{DEFAULT_API_TASK_EXECUTION_CREATION_CONFLICT_RETRY_DELAY_SECONDS}
otherwise.""")
    api_group.add_argument('--api-request-timeout',
            default=DEFAULT_API_REQUEST_TIMEOUT_SECONDS, help=f"""
Timeout for contacting API server, in seconds. Defaults to
{DEFAULT_API_REQUEST_TIMEOUT_SECONDS}.""")
    api_group.add_argument('-o', '--offline-mode', action='store_true',
            help='Do not communicate with or rely on an API server')
    api_group.add_argument('-p', '--prevent-offline-execution',
            action='store_true',
            help='Do not start processes if the API server is unavailable.')
    api_group.add_argument('-d', '--deployment',
            help='Deployment name (production, staging, etc.)')
    api_group.add_argument('--send-pid', action='store_true',
    ...
    log_group.add_argument('-l', '--log-level',
            ...
            help='Log level')
    log_group.add_argument('--log-secrets', action='store_true',
            help='Log sensitive information')

    process_group = parser.add_argument_group('process', 'Process settings')
    process_group.add_argument('-w', '--work-dir', default='.',
            help='Working directory. Defaults to the current directory.')
    process_group.add_argument('-t', '--process-timeout', help="""
Timeout for process, in seconds. -1 means no timeout, which is the default.""")
    process_group.add_argument('-r', '--process-max-retries', default=0,
            help="""
Maximum number of times to retry failed processes. -1 means to retry forever.
Defaults to 0.""")
    process_group.add_argument('--process-retry-delay',
            default=DEFAULT_PROCESS_RETRY_DELAY_SECONDS, help=f"""
Number of seconds to wait before retrying a process. Defaults to
{DEFAULT_PROCESS_RETRY_DELAY_SECONDS}.""")
    process_group.add_argument('--process-check-interval',
            default=DEFAULT_PROCESS_CHECK_INTERVAL_SECONDS, help=f"""
Number of seconds to wait between checking the status of processes.
Defaults to {DEFAULT_PROCESS_CHECK_INTERVAL_SECONDS}.""")
    process_group.add_argument('--process-termination-grace-period',
...
Defaults to {DEFAULT_PROCESS_TERMINATION_GRACE_PERIOD_SECONDS}.""")
    ...
    update_group.add_argument('--enable-status-update-listener',
            action='store_true',
            help=f"""
...
-1 means to not send status updates except with heartbeats. Defaults to -1.""")
    ...
    config_group.add_argument('-e', '--env', action='append',
            dest='env_locations',
            help="""
    ...
    config_group.add_argument('-c', '--config', action='append',
...
to include multiple locations.""")
    ...
    config_group.add_argument('--config-merge-strategy',
            choices=[
...
server)""")
    rollbar_group.add_argument('--rollbar-retries',
            help=f"""
Number of retries per Rollbar request.
Defaults to {DEFAULT_ROLLBAR_RETRIES}.""")
    rollbar_group.add_argument('--rollbar-retry-delay',
...
{DEFAULT_ROLLBAR_RETRY_DELAY_SECONDS}.""")
    rollbar_group.add_argument('--rollbar-timeout',
