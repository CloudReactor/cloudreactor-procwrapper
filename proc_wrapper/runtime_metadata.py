...
def fetch_runtime_metadata(env: Dict[str, str]) -> Optional[RuntimeMetadata]:
    return fetch_ecs_container_metadata(env=env)


def fetch_ecs_container_metadata(env: Dict[str, str]) -> Optional[RuntimeMetadata]:
    task_metadata_url = env.get('ECS_CONTAINER_METADATA_URI_V4') \
    ...
    if task_metadata_url:
        ...
        try:
            ...
            return convert_ecs_task_metadata(parsed_metadata)
...
def convert_ecs_task_metadata(metadata: Dict[str, Any]) -> RuntimeMetadata:
    cluster_arn = metadata.get('Cluster') or ''
    task_arn = metadata.get('TaskARN') or ''
    task_definition_arn = compute_ecs_task_definition_arn(metadata) or ''
    ...
    if az:
        ...
        region = compute_region_from_ecs_cluster_arn(cluster_arn)
...
def compute_region_from_ecs_cluster_arn(cluster_arn: Optional[str]) \
        -> Optional[str]:
    if not cluster_arn:
        return None
...
def compute_ecs_task_definition_arn(metadata: Dict[str, Any]) -> Optional[str]:
