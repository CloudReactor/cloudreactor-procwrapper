...
main_args = main_parser.parse_args(namespace=ProcWrapperParams(
        embedded_mode=False))
...
config_resolver = ConfigResolver(params=main_args,
    runtime_metadata=runtime_metadata)

resolved_env, failed_var_names = config_resolver.fetch_and_resolve_env()

main_args.override_proc_wrapper_params_from_env(resolved_env,
        mutable_only=False, runtime_metadata=runtime_metadata)

ProcWrapper(params=main_args,
        runtime_metadata_fetcher=runtime_metadata_fetcher,
        config_resolver=config_resolver, env_override=resolved_env).run()
