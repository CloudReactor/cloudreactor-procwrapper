...
IMAGE_NAME=cloudreactor-proc-wrapper-pyinstaller-test-linux-amd64

mkdir -p "$SCRIPT_DIR/docker_context_linux_amd64"
cp "$BASE_DIR/bin/pyinstaller/linux-amd64/$VERSION/proc_wrapper" $CONTEXT_DIR
...
export PROC_WRAPPER_TASK_COMMAND='echo "PyInstaller linux-amd64 executable passed!"'
